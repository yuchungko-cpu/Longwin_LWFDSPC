...
#
# ⚠ 而且**掉線後救不回來**：沖洗解析器、自動重開序列埠、使用者手動按「斷線」再按
# 「連線」全部無效，只能重置控制器電源。這一點推翻了 8fc9d1d 當初的前提
# (「使用者按連線就能恢復」) —— 那個觀察是在 115200 上做的。
# 所以 230400 不是「可能比較不穩」，是「卡死之後主機端沒有任何辦法」。
#
# 原因還沒查出來 (TX ISR 與 UTXISEL 都排除過了)。頻寬換穩定是對的取捨：
# 讀不到資料時頻寬多少都沒有意義。
#
...
# 另一個支援的值。頻寬加倍，但見上面：實機上會中途停止回應**而且救不回來**，
# 所以不是預設。留著是因為韌體支援它、而 Scope 擷取確實會快一倍 —— 但選它之前
# GUI 會先跳橫幅講清楚代價 (見 motor_gui._on_baud_pick)。
#
